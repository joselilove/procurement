...
$(document).ready(function () {
    ...
    modification = false;
    $('.ppmp_type_title').text(`${searchParams.get('ppmp_type')} ${searchParams.get('ppmp_year')}`);
    fetch_item();
    ...
    $(document).on('click', '#sumbmit-item', function (e) {
        ...
        $.each($("#ppmp_item option:selected"), function () {
            item.push([searchParams.get('ppmp_id_mod') ,$(this).val(), searchParams.get('ppmp_id')]);
        });
...
function fetch_item() {
    ...
    $.ajax({
        data: data,
        type: 'post',
        url: '/item_mod_ROUTE/fetch_item_route',
...
function insert_ppmp_item(item) {
    ...
    $.ajax({
        data: data,
        type: 'post',
        url: '/item_mod_ROUTE/insert_item_ppmp_route',
...
function fetch_your_ppmp_item_table() {
    ...
    $.ajax({
        data: data,
        type: 'post',
        url: '/item_mod_ROUTE/fetch_your_ppmp_item',
        success: function (result) {
            arrange_item(result)
        }
    });
...
function getCategory() {
    $.ajax({
        type: 'get',
        url: '/item_mod_ROUTE/fetch_category',
        success: function (result) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                data: data,
                type: 'delete',
                url: '/item_mod_ROUTE/delete_item_ppmp_route',
                success: function (result) {
                    Swal.fire(
                        'Deleted!',
    ...
    for (x = 0; x < data.data.length; x++) {
        for (y = 0; y < data.data.length; y++) {
            if (data.data[x].Item_category_FIELD == data.data[y].Item_category_FIELD) {
                ...
                temp_data[count] = {
                    'PPMPItem_id_FIELD': data.data[y].ppmpitem_mod_id,
                    ...
                    'PPMPItem_noteOrRemarks_FIELD': data.data[y].PPMPItem_note,
                    'PPMP_id_FIELD': data.data[y].PPMP_id,
...
function update_ppmp_item_qty(){
    ...
    $.ajax({
        data:data,
        type:'put',
        url:'/item_mod_ROUTE/update_selected_item',
        success:function(result){
            Swal.fire(
                'Success!',
